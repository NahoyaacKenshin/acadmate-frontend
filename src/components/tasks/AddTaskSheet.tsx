...
export function AddTaskSheet({ visible, subjects, onClose }: AddTaskSheetProps) {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      // Write directly to local SQLite — PowerSync will upload to backend automatically
      await powerSync.execute(
        `INSERT INTO Task (id, title, description, dueDate, completed, subjectId, userId, createdAt, updatedAt)
         VALUES (?, ?, ?, ?, 0, ?, ?, ?, ?)`,
        [id, title.trim(), description.trim() || null, dueDate.trim() || null, selectedSubjectId, userId, now, now]
      );
