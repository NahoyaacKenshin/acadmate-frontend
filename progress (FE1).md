...
## 2026-06-24
- **Frontend 1 (Week 1) â€” Design System, Navigation & Auth Screens**:
  ...
  - **Auth Screens:** Polished `LoginForm`, `SignupForm`, and `VerifyEmailPrompt` components â€” centered layout, `Inter` font, `placeholderTextColor`, `Button` component from React Native Reusables, rounded-2xl inputs, consistent spacing and typography.
- **Frontend 2 (Week 1) â€” State & Integration**:
...
## 2026-07-02
- **Frontend 1 (Week 2) â€” Task Manager UI**:
...
- **Frontend 2 (Week 2) â€” Task Manager State & Integration**:
  ...
  - **Reactive Query Hooks**: Created `src/hooks/useTasks.ts` (tasks + subject JOIN) and `src/hooks/useSubjects.ts` â€” both use `useQuery` from `@powersync/react` for live, reactive reads from local SQLite.
...
- **Backend (Week 3) â€” Calendar Core & Holidays**:
  ...
  - **Holiday Data Seed**: Created an idempotent seed script (`prisma/seed.ts`) populating the database with 39 official Philippine holidays for 2026â€“2027.
...
## 2026-07-06
- **Frontend 1 (Week 3) â€” Calendar Screen (Combined Month + Week View)**:
  ...
  - **Reactive Hooks**: Created `src/hooks/useCalendarEvents.ts` and `src/hooks/useClassSchedules.ts` â€” both mirror the existing `useTasks` pattern (PowerSync `useQuery`, LEFT JOIN Subject for color/name, zero REST reads).
  - **MonthGrid Component** (`src/components/calendar/MonthGrid.tsx`): Full 6Ã—7 day-cell grid (week starting Monday). Each cell shows the day number with: a primary-blue fill for the selected day, a primary-blue ring for today, and up to 3 colored event dots (holiday = red/amber, class = subject color, one-off event = subject/event color). Month navigation (prev/next) built-in. Exported the `Holiday` interface for shared use.
  - **WeekStrip Component** (`src/components/calendar/WeekStrip.tsx`): Shows Monâ€“Sun for the selected week with per-day event dots, prev/next week navigation, and a primary-blue chevron toggle button to expand/collapse the month grid above it.
  - **DayView Component** (`src/components/calendar/DayView.tsx`): Scrollable event list for the selected date, split into three labeled sections â€” "Classes" (recurring, with F2F/ONLINE/HYBRID modality badge, room, time range), "Events" (one-off CalendarEvents), and "Tasks Due" (tasks with matching dueDate). Includes a holiday banner (red for REGULAR, amber for SPECIAL) and a clean empty state.
  - **AddEventSheet** (`src/components/calendar/AddEventSheet.tsx`): Bottom-sheet modal matching the AddTaskSheet style. Fields: title, description, all-day toggle, start/end datetime pickers (same Android dateâ†’time chain / iOS inline spinner pattern), location, 6-swatch color picker, subject dropdown. Writes to `CalendarEvent` via `powerSync.execute()` (offline-first, syncs upstream when online).
  ...
  - **Calendar `+` Action Menu** (`app/(app)/calendar.tsx`): Replaced the single `+` button with a contextual dropdown that appears on tap, offering two options â€” "Add Event" (blue CalendarDays icon) and "Add Class" (green BookOpen icon) â€” each with a subtitle. Tapping outside the menu or selecting an option dismisses it cleanly.

## 2026-07-09
- **Frontend 1 (Week 3) â€” Exam Week Support**:
...
- **Bug Fix â€” PowerSync Token Auto-Refresh** (`src/db/PowerSyncConnector.ts`):
...
- **Frontend 2 (Week 3) â€” State & Integration (Calendar & Offline Sync)**:
...
## 2026-07-16
- **Backend (Week 3) â€” Prisma & Sync Bug Fixes**:
...
- **Frontend (Week 3) â€” React Native Deprecation Fixes**:
...
- **Backend (Week 4) â€” AI Schedule Parsing Pipeline**:
  - **Gemini Utility** (`src/utils/gemini.ts`): Set up `GoogleGenerativeAI` client. Built `generateWithFallback()` â€” a defensive cascade handler that tries `gemini-pro-latest` first, then `gemini-flash-latest`, then `gemini-flash-lite-latest` on successive `429` rate limit errors. All three models use a single `GEMINI_API_KEY`.
...
- **Frontend 1 (Week 4) â€” AI Schedule Scan UI**:
  - **Packages**: Installed `expo-document-picker` and `expo-image-picker` (Expo SDK 56 compatible) for file and camera access.
  - **`AILoadingOverlay.tsx`** (`src/components/schedule/AILoadingOverlay.tsx`): Full-screen modal overlay shown while AI is reading a document. Features a pulsing glow ring, a slow-rotating arc spinner, a `Sparkles` icon, and a dot loader. Status text cycles through friendly messages: "Reading your documentâ€¦" â†’ "Detecting your scheduleâ€¦" â†’ "Organising the resultsâ€¦" â†’ "Almost doneâ€¦". All animations use the React Native `Animated` API.
  ...
  - **`ParsedItemRow.tsx`** (`src/components/schedule/ParsedItemRow.tsx`): Three row components (`ClassScheduleRow`, `CalendarEventRow`, `ExamWeekRow`) for the Review screen. Each row shows an icon, formatted details, and a dismiss (`Ã—`) button. `ClassScheduleRow` includes a yellow **"+ New Subject"** badge for subjects not yet in the local database.
  - **`schedule-upload.tsx`** (`app/(app)/schedule-upload.tsx`): Full upload screen. Three picker cards (PDF/DOCX, gallery, camera) with permission handling. Selected file shows a live preview card. The **"Read My Schedule â†’"** CTA calls `POST /api/schedule-parser/parse` with `multipart/form-data`, shows the `AILoadingOverlay`, then navigates to the review screen on success. Friendly error banners for network, file-size, and unsupported-type errors.
  - **`schedule-confirm.tsx`** (`app/(app)/schedule-confirm.tsx`): Review screen showing AI-detected items in three collapsible sections (Classes, Events & Holidays, Exam Periods). Each item can be individually removed with an `Ã—` button. Unrecognised subjects trigger an inline **"Create New Subject"** bottom-sheet (name pre-filled from the AI result, colour picker included) that writes directly to the local `Subject` table via `powerSync.execute()`. **"Add to My Calendar"** CTA is stubbed with `resolvedClasses`/`events`/`exams` ready for FE2 to hook in.
  ...
  - **`calendar.tsx`**: Added a 4th action-menu item â€” **"Scan Schedule"** (purple `ScanLine` icon, subtitle "Upload PDF, photo or Word doc") â€” that navigates to `/(app)/schedule-upload`.

- **Frontend 2 (Week 4) â€” State & Integration (AI Schedule Parsing)**:
...
## 2026-07-17
- **Frontend 2 (Week 4) â€” File Upload Stability Fix**:
...
- **Backend (Week 4) â€” Gemini Rate Limit Cascade Fix**:
...
- **Frontend 1 (Week 4) â€” Worklet Crash Fix**:
...
## 2026-07-18
- **Frontend 1 & 2 (Week 4) â€” Deep Editing Capabilities**:
...
- **Bug Fixes â€” TypeScript Errors & Logic**:
...
## 2026-07-18
- **Backend (Week 4 Additional â€” Admin System & Set A/B Scheduling Logic)**:
  - **Prisma Schema**: Added two new globally-shared models â€” `SemesterRule` (admin-defined Saturday Set A/B rules with `date`, `ruleType` enum, and optional `label`) and `ProgramMapping` (maps degree program names like "BSIT" to `StudentSet` enum A or B). Added `StudentSet` and `SemesterRuleType` enums. Ran and applied migration `20260718154237_add_semester_rules_and_program_mappings` to Neon Postgres.
  - **Prisma Client**: Regenerated Prisma Client (`prisma generate`) to expose the new `semesterRule` and `programMapping` accessors.
  - **Admin Middleware**: Created `src/middlewares/admin-middleware.ts` â€” stacks after `AuthMiddleware` and rejects requests with a 403 if the JWT payload's role is not `ADMIN`.
  ...
  - **AI Parsing Pipeline**: Refactored `src/services/schedule-parser.service.ts` â€” replaced the static `SYSTEM_PROMPT` constant with a `buildSystemPrompt(studentSet?)` factory function. When `studentSet` is "A" or "B", it appends a directive instructing Gemini to extract only the relevant Set's room from class schedule images (e.g. Set A columns only). Updated `parseScheduleFromFile` signature to accept an optional `studentSet` parameter. Updated `src/controllers/schedule-parser.controller.ts` to read `studentSet` from the multipart form body and pass it through.

## 2026-07-25
- **Frontend 1 (Week 4 Additional â€” Admin System UI & Onboarding)**:
...
- **Frontend 2 (Week 4 Additional â€” Schedule Resolver Utility & PowerSync Sync)**:
...
- **Frontend 1 & 2 (Week 4 Additional â€” Dynamic Programs & Admin-Only Exam Weeks)**:
...
- **Frontend 1 & 2 (Week 4 Additional â€” Auth Routing, Onboarding Gating & Dedicated Admin Layout)**:
...
- **Calendar & Schedule Blockers Fix**:
  ...
  - **Frontend (`DayView.tsx`, `MonthGrid.tsx`, `WeekStrip.tsx`)**: Updated calendar components to pass combined holidays to `isScheduleActiveOnDate`. Added Exam Week banner (`EXAM WEEK â€” title`) and Class Suspension banner (`CLASS SUSPENSION â€” name`) to `DayView` when active on the selected date.
...
## 2026-08-03
- **Backend (Week 5) â€” AI Notebook: File Upload & Automated Processing Pipeline**:
  ...
  - **Prisma Schema** (`prisma/schema.prisma`): Added three new models and two enums:
    ...
    - `Notebook`: id, title, description, userId â†’ User, sources â†’ Source[]. Indexed on `userId`.
    - `Source`: id, fileName, fileType, storagePath (Supabase object path), status, rawText (up to 100k chars), errorMsg, notebookId â†’ Notebook, userId â†’ User, chunks â†’ SourceChunk[].
    - `SourceChunk`: id, chunkIndex, content (~500 words), `embedding Unsupported("vector(768)")`, sourceId â†’ Source, notebookId. Embeddings stored via pgvector raw SQL.
  ...
  - **Notebook Service** (`src/services/notebook.service.ts`): Full async processing pipeline:
    1. `uploadSourceFile()` â€” uploads file to Supabase Storage (`notebook-sources` bucket), creates `Source` (status: `PENDING`), fires background `processSource()` without awaiting.
    2. `processSource()` â€” sets status `PROCESSING` â†’ extracts text (pdf-parse / mammoth / Gemini Vision OCR / raw buffer) â†’ normalizes â†’ chunks (~500 words, 50-word overlap) â†’ embeds each chunk via `text-embedding-004` â†’ inserts `SourceChunk` rows with pgvector via `prisma.$executeRaw` â†’ sets status `READY`. On any failure sets status `FAILED` with error message.
    3. `deleteSource()` â€” removes file from Supabase Storage and deletes Source record (cascades SourceChunks).
  - **Notebook Controller** (`src/controllers/notebook.controller.ts`): Full REST handlers â€” `listNotebooks`, `createNotebook`, `getNotebook`, `deleteNotebook`, `uploadSource`, `deleteSourceHandler`. All routes require `AuthMiddleware`. Source listing includes `_count.chunks` for progress display.
  ...
  - **TypeScript**: Clean `tsc --noEmit` â€” zero errors.

## 2026-08-03 (continued)
- **Frontend 1 (Week 5) â€” AI Notebook: File Upload & Processing Pipeline**:
  ...
  - **`SourceListItem.tsx`** (`src/components/notebook/SourceListItem.tsx`): Row component for an uploaded source. Shows file type icon (PDF=red, Image=purple, Text=gray), filename, status badge (`PENDING`â†’amber, `PROCESSING`â†’blue, `READY`â†’green, `FAILED`â†’red), chunk count when `READY`, and trash delete button with confirmation Alert.
...
## 2026-08-03 (continued)
- **Frontend 2 (Week 5) â€” Notebook State Management & Upload Streams**:
...
- **Frontend 1 & 2 (Week 8) - Local Offline-First Notifications Engine & Study Reminders Complete**:
  - **Expo Notifications Trigger Fixes** (`src/services/notificationService.ts`):
    - Fixed schema bug where scheduling triggers were missing explicit `Notifications.SchedulableTriggerInputTypes.WEEKLY` and `Notifications.SchedulableTriggerInputTypes.DATE` types, which caused runtime schedule rejections on mobile devices.
    - Added midnight wraparound handling in `scheduleClassReminders` ensuring `day_of_week` properly decrements when class lead time shifts before midnight.
    - Updated `rescheduleAllClasses` to respect student set assignments (`A`, `B`, or `BOTH`), preventing students from receiving notifications for other set classes.
    - Added `sendTestNotification(delaySeconds)` helper to enable instant on-device verification from settings.
  - **In-App Toast Banner Component** (`src/components/common/InAppNotificationBanner.tsx`):
    - Built a custom top-floating animated glassmorphic banner alert utilizing `react-native-reanimated` that listens to `Notifications.addNotificationReceivedListener` when the app is in the foreground.
    - Features type-specific color accents (`#6C8EFF` for classes, `#10B981` for tasks, `#A78BFA` for study, `#F59E0B` for tests), rich typography, spring entry animations, swipe/tap auto-dismiss (4.5s), and deep-linking upon tap.
    - Mounted directly inside `NotificationProvider` (`src/providers/NotificationProvider.tsx`) for app-wide availability.
  - **Notification Preferences & Test Controls** (`app/(app)/settings.tsx`):
    - Added **Exam Week Alerts** toggle option connected to `prefs.examAlerts`.
    - Added an interactive **Trigger Test Notification (3s)** button allowing instant testing of native notification delivery, sound, and foreground toast banners with a single tap.
  - **Notebook Study Scheduler Lead-Time Support** (`src/components/notebook/StudySchedulerModal.tsx`):
    - Added interactive reminder lead-time selection pills (`At start`, `15m before`, `30m before`, `1h before`).
    - Computed precise alert fire timestamps and passed lead-time metadata into local notification payloads.
- **Backend**: Updated `schedule-parser.service.ts` to accept an optional `currentSchedule` context string. A `MERGE_PROMPT_SUFFIX` function injects it into the Gemini prompt, instructing the model to update fields on existing items (e.g. Set A/B) and add new ones without duplicates.
  - **Backend**: Migrated `uploadMiddleware` from `.single("file")` to `.any()` so the `currentSchedule` text field can be received alongside the file.
  - **Frontend**: Created `src/hooks/useScheduleScanner.ts` - a reusable hook encapsulating all file-picker logic, state, and the `FileSystem.uploadAsync` pipeline, with merge mode support.
  - **Frontend**: Refactored `schedule-upload.tsx` to use `useScheduleScanner`, significantly reducing its size.
...
- **Frontend 1 & 2 (Week 8) - Local Offline-First Notifications Engine & Study Reminders Complete**:
  ...
  - **Notebook Study Scheduler Lead-Time Support** (`src/components/notebook/StudySchedulerModal.tsx`):
    - Added interactive reminder lead-time selection pills (`At start`, `15m before`, `30m before`, `1h before`).
    - Computed precise alert fire timestamps and passed lead-time metadata into local notification payloads.
  - **Cold-Start Deep-Linking** (`src/hooks/useNotificationDeepLink.ts`):
    - Enhanced deep-link listener to invoke `Notifications.getLastNotificationResponseAsync()` on mount, correctly routing users when the app is launched from a closed/terminated state by tapping a notification banner.
  - **Native Android Configuration & Manifest Permissions** (`app.json`, `android/app/src/main/AndroidManifest.xml`):
    - Added `"expo-notifications"` plugin configuration with `#6C8EFF` color scheme in `app.json`.
    - Added required Android permissions in `AndroidManifest.xml`: `POST_NOTIFICATIONS` (Android 13+ / API 33+ requirement), `SCHEDULE_EXACT_ALARM`, `USE_EXACT_ALARM`, and `RECEIVE_BOOT_COMPLETED`.
  - **TypeScript Verification & Strict Compilation**:
    - Resolved corrupted node_modules type definitions, configured `skipLibCheck` and `exclude` paths in `tsconfig.json`.
    - Executed `npm run typecheck` with 0 errors.

## 2026-08-26
- **Frontend 1 & 2 — Date & Time Picker Android Fix & React Hook Order Resolution**:
  - **React Hook Order Fix (`app/(app)/index.tsx`)**:
    - Resolved runtime crash `"Rendered more hooks than during the previous render"` triggered when logging out of an Admin account.
    - Root cause: `HomeScreen` was executing an early conditional return (`if (user?.role === 'ADMIN') return <AdminDashboardScreen />;`) before calling the remaining task and schedule hooks. On logout, the hook count changed between renders.
    - Extracted all student data hooks into a dedicated `StudentHomeScreen` component so both Admin and Student paths maintain fixed, unconditional hook executions.
  - **Android Date & Time Picker Architecture Fix (App-Wide)**:
    - Resolved issues where setting time on any date/time field auto-reverted to default/random values and where exiting the picker caused it to immediately re-open without user interaction.
    - Root cause: On Android, declarative `<DateTimePicker />` in JSX triggered re-entrance loops via internal `useEffect` when state or function references updated. Additionally, `onValueChange` did not intercept dismissal events and `dateFromHHMM` regenerated millisecond timestamps on every frame.
    - Standardized all Android date/time interactions to the official imperative **`DateTimePickerAndroid.open()`** API:
      - Dialogs open strictly on user press with zero background re-renders in JSX.
      - Explicit `event.type === 'set'` checks ensure only confirmed dates/times are committed to state, while `event.type === 'dismissed'` cleanly cancels without modifying state or re-opening.
      - Chained pickers (Date ? Time) seamlessly chain directly within `DateTimePickerAndroid.open()` callbacks.
      - `dateFromHHMM` standardized to a constant baseline date (`new Date(2000, 0, 1, h, m, 0, 0)`).
    - Refactored components:
      - `src/components/tasks/AddTaskSheet.tsx`
      - `src/components/tasks/EditTaskSheet.tsx`
      - `src/components/calendar/AddEventSheet.tsx`
      - `src/components/calendar/EditEventSheet.tsx`
      - `src/components/calendar/AddClassSheet.tsx`
      - `src/components/calendar/EditClassSheet.tsx`
      - `src/components/schedule/EditParsedSheets.tsx` (all 3 sheets: Class, Event, Exam Week)
      - `src/components/notebook/StudySchedulerModal.tsx`
      - `src/components/calendar/AddExamWeekModal.tsx`
      - `src/components/admin/AddSemesterRuleModal.tsx`
    - Verified iOS inline/spinner pickers remain cleanly rendered inside JSX with explicit "Done" button dismissals.
